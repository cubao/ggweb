...
dist:
	mkdir -p dist
	cp build-em/bin/ggweb-app.* dist
	cp build-em/bin/ggweb-app-public/* dist
.PHONY: em_build dist
...
DOCKER_TAG_GGWEB ?= ghcr.io/cubao/build-env-ggweb:v0.0.1
docker_build:
	docker build -t $(DOCKER_TAG_GGWEB) -f Dockerfile .
docker_push:
	docker push $(DOCKER_TAG_GGWEB)
